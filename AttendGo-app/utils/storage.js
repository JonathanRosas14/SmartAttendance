...
export async function eliminarClavesAttendGo() {
  try {
    if (isWeb) {
      ATTENDGO_KEYS.forEach((key) => localStorage.removeItem(key));
      return;
    }

    await AsyncStorage.multiRemove(ATTENDGO_KEYS);
  } catch (error) {
    console.error("❌ Error limpiando claves AttendGo:", error);
  }
}
